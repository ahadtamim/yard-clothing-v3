...
export default function Checkout() {
  ...
  // FIXED: Calculate total using quantity for accuracy
  const total = items.reduce((acc: number, item: any) => acc + (Number(item.price || 0) * (item.quantity || 1)), 0)

  useEffect(() => {
    setMounted(true)
    // Only redirect if mounted and definitely empty
  ...
  // Prevent flash of empty page or hydration error
    ...
    <div className="max-w-4xl mx-auto pt-32 pb-20 px-6 text-black min-h-screen">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
        ...
        <div>
          ...
          <form onSubmit={handleConfirmOrder} id="checkout-form" className="space-y-8">
            <div className="group">
              ...
              <input required type="email" placeholder="EMAIL FOR ORDER RECEIPT" className="w-full border-b border-gray-200 py-3 outline-none text-xs tracking-widest focus:border-black transition-colors bg-transparent text-black" />
            </div>
            
            <div className="group">
              <label className="text-[9px] uppercase font-bold text-gray-400 tracking-widest mb-1 block">Full Name</label>
              <input required type="text" placeholder="FULL NAME" className="w-full border-b border-gray-200 py-3 outline-none text-xs tracking-widest focus:border-black transition-colors bg-transparent text-black" />
            </div>

            <div className="group">
              <label className="text-[9px] uppercase font-bold text-gray-400 tracking-widest mb-1 block">Address</label>
              <textarea required placeholder="SHIPPING ADDRESS (STREET, CITY, ZIP)" rows={3} className="w-full border-b border-gray-200 py-3 outline-none text-xs tracking-widest focus:border-black transition-colors bg-transparent text-black resize-none" />
            </div>
          </form>
        </div>
        ...
        <div className="bg-gray-50 p-8 rounded-sm h-fit border border-gray-100">
          ...
          <div className="mb-10">
            <h3 className="text-[9px] uppercase tracking-widest font-black mb-4 text-black">Payment Method</h3>
